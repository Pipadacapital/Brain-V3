-- 0128_list_shiprocket_connectors_for_webhook.sql
--
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION '0128 failed: list_shiprocket_connectors_for_webhook must be SECURITY DEFINER + search_path-pinned';
  END IF;
  IF NOT has_function_privilege('brain_app', 'list_shiprocket_connectors_for_webhook()', 'EXECUTE') THEN
    RAISE EXCEPTION '0128 failed: brain_app lacks EXECUTE on list_shiprocket_connectors_for_webhook()';
  END IF;
END $$;
