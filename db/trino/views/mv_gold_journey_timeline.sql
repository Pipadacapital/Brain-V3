-- ============================================================
-- Brain V4 — Trino serving VIEW: mv_gold_journey_timeline
--
...
--
-- The metric-engine reads this as the two-part name brain_serving.mv_gold_journey_timeline;
-- with the Trino default catalog = iceberg that resolves to
-- iceberg.brain_serving.mv_gold_journey_timeline. brand_id is the tenant key; the
-- ${BRAND_PREDICATE} seam injects brand_id = ? at read time.
-- ============================================================
CREATE OR REPLACE VIEW iceberg.brain_serving.mv_gold_journey_timeline AS
SELECT
  brand_id,
  brain_anon_id,
  touch_seq,
