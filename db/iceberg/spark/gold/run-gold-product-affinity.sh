#!/usr/bin/env bash
# run-gold-product-affinity.sh — Brain V4 P3 (Spark Gold): build the NET-NEW gap Gold mart
# gold_product_affinity (frequently-bought-together / co-purchase pairs; no dbt predecessor → parity status=NEW)
# FROM Iceberg Silver via Spark. ADDITIVE + idempotent + re-runnable; repoints NO reader.
#
#   gold_product_affinity.py  (silver_order_line → same-order self-join → per (brand, product_a<product_b)
#                              co_purchase_count + support_pct, top-N partners per product)
#
...
# Requires the lakehouse profile (iceberg-rest + minio) up + Silver built (silver_order_line).
#
# Usage:   db/iceberg/spark/gold/run-gold-product-affinity.sh
...
echo "[gold-product-affinity] image=${SPARK_IMAGE} netns=container:${REDPANDA_CONTAINER}"
echo "[gold-product-affinity] packages=${PACKAGES}"

docker volume create brain-spark-ivy >/dev/null

echo "[gold-product-affinity] >>> spark-submit gold_product_affinity.py"
docker run --rm \
  ...
  -e TOP_PAIRS_PER_PRODUCT="${TOP_PAIRS_PER_PRODUCT:-50}" \
  ...
  /opt/spark/bin/spark-submit \
    ...
    "/opt/spark-src/gold/gold_product_affinity.py"

echo "[gold-product-affinity] DONE ✓"
