#!/usr/bin/env bash
# run-gold-campaign-attribution.sh — Brain V4 Phase 2 (Spark Gold): build the NET-NEW gap Gold mart
# gold_campaign_attribution (time-to-2nd-purchase RETENTION LATENCY; no dbt predecessor → parity status=NEW)
# FROM Iceberg Silver via Spark. ADDITIVE + idempotent + re-runnable; repoints NO reader.
#
#   gold_campaign_attribution.py  (gold_attribution_credit ⋈ gold_campaign_performance/silver_marketing_spend → per-campaign
#                            attributed_revenue_minor + spend_minor + roas_bps)
#
...
# Requires the lakehouse profile (iceberg-rest + minio) up + Silver+attribution built.
#
# Usage:   db/iceberg/spark/gold/run-gold-campaign-attribution.sh
...
echo "[gold-campaign-attribution] image=${SPARK_IMAGE} netns=container:${REDPANDA_CONTAINER}"
echo "[gold-campaign-attribution] packages=${PACKAGES}"
...
docker run --rm \
  ...
  /opt/spark/bin/spark-submit \
    ...
    "/opt/spark-src/gold/gold_campaign_attribution.py"

echo "[gold-campaign-attribution] DONE ✓"
