...
def build(spark: SparkSession):
    ...
    # Skip-guard: empty-lane skip — the ga4_rows_raw_connect table auto-creates on the lane's
    # first record (ADR-0010). No source rows → nothing to normalize; return cleanly instead of
    # failing the struct-column select on the empty placeholder frame.
    if raw.limit(1).count() == 0:
        print(f"[silver-ga4-normalize] {rn.connect_source_table(CATALOG, BRONZE_NAMESPACE, 'ga4_rows_raw')} has 0 rows — skipping (empty lane; table auto-creates on first record, ADR-0010)", flush=True)
        return TARGET, 0
    ...
    out = canon.select(
        ...
        _event_category(lit("ga4.session.v1")).alias("event_category"),
