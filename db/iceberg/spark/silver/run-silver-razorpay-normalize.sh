#!/usr/bin/env bash
# run-silver-razorpay-normalize.sh — ADR-0006 P4: normalize RAW Razorpay settlement items in Spark Silver.
# Reads brain_bronze.razorpay_settlement_raw (Kafka Connect Iceberg sink output) and produces the canonical
# settlement.live.v1 rows via the GOLDEN-VECTOR-VERIFIED shared ports in _raw_normalize.py (+ Razorpay-local
# pure helpers). Writes a SHADOW table by default (dual-run parity); set TARGET_TABLE=silver_collector_event
# at the settlement-lane cutover. Requires the lakehouse profile + Postgres (per-brand salt for C1 hashing).
set -euo pipefail
...
docker run --rm \
  ...
  -e TARGET_TABLE="${TARGET_TABLE:-silver_collector_event_razorpay_shadow}" \
  -e RAZORPAY_RAW_NEST="${RAZORPAY_RAW_NEST:-settlement}" \
...
echo ""
echo "[silver] DONE — silver_razorpay_normalize (raw Razorpay settlement items → canonical Silver) ✓"
