/**
 * ad-spend-metrics.live.test.ts — Live Postgres tests for the Track 3 spend/ROAS metrics.
 *
 * Proves (the heart of Track 3 — see 05-architecture.md Track 3):
 *
 *   1. PARITY ORACLE (ad_spend, sole-read-path, D-3):
 *      Seed spend rows for BRAND_A; call computeAdSpendTimeseries (engine) AND
 *      getAdSpendTimeseries (BFF use-case); the summed bucket spend EXACTLY equals
 *      the ad_spend_as_of() seam SUM — exact BIGINT, no rounding, no ad-hoc SUM.
 *
 *   2. HONEST-EMPTY (D-2):
 *      A brand with ZERO ad_spend_ledger rows → state='no_data' (both endpoints).
 *      blended-roas with no spend → no_data (no denominator → honest).
 *
 *   3. CROSS-CURRENCY GUARD (same-currency-only, ADR Track 3):
 *      Spend in INR + USD never blends; blended_roas reports per-currency rows and
 *      NEVER divides realized(INR) by spend(USD).
 *
 *   4. HONEST spend=0 → roas_ratio=null:
 *      A currency with realized but no spend in window → roas_ratio is null, never ∞/0.
 *
 *   5. RLS ISOLATION under brain_app (FORCE-RLS, non-inert):
 *      Seed BRAND_A spend (superPool); read under appPool GUC=BRAND_B → no_data.
 *      Negative control: GUC=BRAND_B querying BRAND_A rows → count 0. Asserts
 *      current_user='brain_app' (dev superuser 'brain' bypasses RLS — would be inert).
 *
...
/** Seed an ad_spend_ledger row via superuser (DDL/seed path). */
...
// ── 1. PARITY ORACLE — engine timeseries SUM == ad_spend_as_of seam SUM (D-3) ──

describe('1. parity oracle — ad_spend engine==seam exact-bigint (sole-read-path, D-3)', () => {
  const from = daysAgoStr(10);
  const to = todayStr();

  beforeAll(async () => {
    await clearSpend(BRAND_A);
    await seedSpend(BRAND_A, { platform: 'meta', levelId: 'c1', statDate: daysAgoStr(5), spendMinor: 100000n, currency: 'INR' });
    await seedSpend(BRAND_A, { platform: 'meta', levelId: 'c2', statDate: daysAgoStr(3), spendMinor: 50000n, currency: 'INR' });
    await seedSpend(BRAND_A, { platform: 'google_ads', levelId: 'g1', statDate: daysAgoStr(2), spendMinor: 30000n, currency: 'INR' });
  });
  afterAll(async () => { await clearSpend(BRAND_A); });

  it('engine timeseries total == ad_spend_as_of seam SUM (exact, no rounding)', async () => {
    ...
    // The seam SUM (the sole as-of read) — read directly under GUC for the oracle.
    ...
    expect(engineTotal).toBe(seamTotal); // sole-read-path proof — would FAIL on ad-hoc SUM divergence
  });
...
describe('2. honest-empty — zero spend rows → no_data', () => {
  beforeAll(async () => { await clearSpend(BRAND_A); await clearRevenue(BRAND_A); });

  it('ad-spend-timeseries with no rows → state=no_data', async () => {
    const result = await getAdSpendTimeseries(
      BRAND_A,
  ...
  it('blended-roas with no spend → state=no_data (no denominator → honest)', async () => {
    // Even with realized revenue present, no spend means no ROAS.
...
// ── 3 + 4. BLENDED ROAS: same-currency-only + honest spend=0→null ─────────────
...
describe('5. RLS isolation under brain_app — cross-brand spend=no_data (non-inert)', () => {
  beforeAll(async () => {
    await clearSpend(BRAND_A);
    await clearSpend(BRAND_B);
    await seedSpend(BRAND_A, { platform: 'meta', levelId: 'iso1', statDate: daysAgoStr(2), spendMinor: 777000n, currency: 'INR' });
  });
  afterAll(async () => { await clearSpend(BRAND_A); await clearSpend(BRAND_B); });

  it('current_user is brain_app (non-superuser, NOBYPASSRLS) — isolation is non-inert', async () => {
  ...
  it('BRAND_A spend visible to BRAND_A (positive control), invisible to BRAND_B', async () => {
    ...
    expect(b.state).toBe('no_data'); // RLS filters BRAND_A's rows out for BRAND_B
  });

  it('[negative-control] GUC=BRAND_B querying BRAND_A spend rows → count 0 (RLS non-inert)', async () => {
    const client = await appPool.connect();
    let count: number;
    try {
      await client.query('BEGIN');
