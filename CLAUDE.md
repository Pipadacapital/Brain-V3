...
- The transform runner is `db/iceberg/duckdb/run_all.py` (ADR-0016 P2): it attaches the Iceberg catalog ONCE and runs every job's `__main__` against one shared warm DuckDB connection (`run_all.py {silver,gold}` single-shot; `run_all.py resident` is the warm micro-batch worker that loops keystone→silver→node-identity→gold every `TRANSFORM_TICK_MS` under a pg advisory lock). The old 90-spawn bash orchestrator loop + its `sparkV4.singleProcess` flag are RETIRED — never reintroduce a per-job `python …` spawn loop. Locally, refresh the medallion with `tools/dev/duckdb-refresh.sh` (a thin dev shim that chains `run_all.py silver → node identity → run_all.py gold → cache-bust`; `STAGE=` overrides keystone/silver/identity/gold); in-cluster it is the `v4-medallion` CronWorkflow (superseded by the `transformWorker` resident Deployment when enabled). The transform jobs are `db/iceberg/duckdb/{silver,gold}/*.py`; maintenance is `db/iceberg/duckdb/maintenance/**`; serving views are `db/iceberg/duckdb/views/*.sql`.
