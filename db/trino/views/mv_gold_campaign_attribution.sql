-- ============================================================
-- Brain V4 — Trino serving VIEW: mv_gold_campaign_attribution
--
-- Brain V4 serving runs over TRINO (Iceberg). This view is the per-CAMPAIGN, per-MODEL
...
--
-- The metric-engine reads this as the two-part name brain_serving.mv_gold_campaign_attribution;
-- with the Trino default catalog = iceberg that resolves to
-- iceberg.brain_serving.mv_gold_campaign_attribution. brand_id is the tenant key; the
-- ${BRAND_PREDICATE} seam injects brand_id = ? at read time.
-- ============================================================
CREATE OR REPLACE VIEW iceberg.brain_serving.mv_gold_campaign_attribution AS
SELECT
  brand_id,
  platform,
  campaign_id,
