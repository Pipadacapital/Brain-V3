-- ============================================================
-- Brain V4 — Trino serving VIEW: mv_gold_cohort_member
--
-- Brain V4 serving runs over TRINO (Iceberg). This view is a THIN projection over the
...
--
-- The metric-engine reads this as the two-part name brain_serving.mv_gold_cohort_member;
-- with the Trino default catalog = iceberg that resolves to
-- iceberg.brain_serving.mv_gold_cohort_member. brand_id is the tenant key; the
-- ${BRAND_PREDICATE} seam injects brand_id = ? at read time.
-- ============================================================
CREATE OR REPLACE VIEW iceberg.brain_serving.mv_gold_cohort_member AS
SELECT
  brand_id,
  customer_key,
  cohort_month,
