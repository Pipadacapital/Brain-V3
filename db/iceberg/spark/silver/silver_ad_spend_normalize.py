...
def event_id_spend_live(brand_id, platform, stat_date, level, level_id, breakdown_key=""):
    """uuidV5FromSpendRow(brandId, platform, statDate, level, levelId, breakdownKey) — REUSES the SHARED
    uuid_shaped port. Seed (ADR-AD-5 + FIREHOSE §2.A):
      `${brandId}:${platform}:${statDate}:${level}:${levelId}:${breakdownKey}:spend.live.v1`.
    breakdownKey='' for the base pass keeps base-grain event_ids BYTE-UNCHANGED (zero re-dedup churn);
    a breakdown pass folds its dims (canonical_breakdown_key) so base + breakdown rows never collide."""
    bk = breakdown_key or ""
    return rn.uuid_shaped(
        f"{brand_id}:{platform}:{stat_date}:{level}:{level_id}:{bk}:spend.live.v1"
    )
...
def build_google(spark: SparkSession):
    ...
    canon = (
        ...
        # COMMON: Google base lane uses breakdown_key='' (Impl-G folds its segment dims in its own PR).
        .withColumn("breakdown_key", lit(""))
        .withColumn("occurred_at_iso", u_stat_iso(col("stat_date")))
        .withColumn(
            "event_id",
            u_eid(
