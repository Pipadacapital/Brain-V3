...
# gokwik.awb_status.v1 is server-trusted AND ledger-fed (the order.live.v1 precedent): the ledger
# bridge consumes it for cod_rto_clawback/cod_delivery_confirmed AND it is landed in Bronze so the
# rich shipment detail (status, terminal_class, pincode, courier) is preserved for silver_shipment
# (Slice 2). shiprocket.shipment_status.v1 joins this set when its connector PR merges.
SERVER_TRUSTED_BRONZE = {"order.live.v1", "shopflo.checkout_abandoned.v1", "gokwik.rto_predict.v1", "gokwik.awb_status.v1"}
