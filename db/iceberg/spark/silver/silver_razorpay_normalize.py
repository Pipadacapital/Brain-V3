...
def build(spark: SparkSession):
    ...
    raw = rn.read_bronze(spark, CATALOG, BRONZE_NAMESPACE, "razorpay_settlement_raw", "razorpay")
    # Skip-guard: empty-lane skip — the razorpay_settlement_raw_connect table auto-creates on the
    ...
    if raw.limit(1).count() == 0:
        print(f"[silver-razorpay-normalize] {rn.connect_source_table(CATALOG, BRONZE_NAMESPACE, 'razorpay_settlement_raw')} has 0 rows — skipping (empty lane; table auto-creates on first record, ADR-0010)", flush=True)
        return TARGET, 0
