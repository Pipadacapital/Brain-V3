...
async function emitRows(
  ...
  if (messages.length > 0) {
    ...
    // deterministic (uuidV5FromGa4Row over the report dimensions), so a re-pull overlap re-mints
    // the SAME (brand_id, event_id) and is collapsed by Bronze compaction dedup + the Silver
    // MERGE (keep-latest also lands late GA4 metric restatements for the same row).
