...
| --- | --- | --- |
| `brain/prod/k8s/core-env` | `core-env` (ns `core` **and** ns `argo`) | `DATABASE_URL` + `BRAIN_APP_DATABASE_URL` (host = `pgbouncer:6432`, **not** Aurora directly), `DATABASE_URL_DIRECT` (direct Aurora — the migration PreSync Job, AUD-COST-011), `REDIS_URL`, `KAFKA_BROKERS`, `TRINO_HOST`, `ICEBERG_REST_URI`, `AWS_REGION=ap-south-1`, `CHECKPOINT_LOCATION` (durable `s3a://`), `COLLECTOR_TOPIC`/`BACKFILL_TOPIC`/`TOPIC_ENV_PREFIX`, `NEO4J_URI`/`NEO4J_USER`/`NEO4J_PASSWORD` (see AUD-COST-006), `AUDIT_CHECKPOINT_BUCKET`, connector/AI-gateway secrets |
