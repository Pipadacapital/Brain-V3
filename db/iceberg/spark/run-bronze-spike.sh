...
# local lakehouse (compose `lakehouse` profile must be up: iceberg-rest + minio + redpanda).
#
...
# host clients) is reachable; service-name DNS (iceberg-rest, minio) still resolves because Redpanda
# is on the compose network. Avoids a dual-listener change to the shared redpanda service.
