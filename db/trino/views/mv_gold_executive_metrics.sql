-- ============================================================
-- Brain V4 — Trino serving VIEW: mv_gold_executive_metrics
--
...
--
-- The metric-engine reads this as the two-part name brain_serving.mv_gold_executive_metrics;
-- with the Trino default catalog = iceberg that resolves to
-- iceberg.brain_serving.mv_gold_executive_metrics. brand_id is the tenant key; the
-- ${BRAND_PREDICATE} seam injects brand_id = ? at read time.
-- ============================================================
CREATE OR REPLACE VIEW iceberg.brain_serving.mv_gold_executive_metrics AS
SELECT
  brand_id,
  currency_code,
  total_orders,
