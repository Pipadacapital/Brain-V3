-- ============================================================
-- Brain V4 — Trino serving VIEW: mv_gold_customer_health
--
...
--
-- The metric-engine reads this as the two-part name brain_serving.mv_gold_customer_health;
-- with the Trino default catalog = iceberg that resolves to
-- iceberg.brain_serving.mv_gold_customer_health. brand_id is the tenant key; the
-- ${BRAND_PREDICATE} seam injects brand_id = ? at read time.
-- ============================================================
CREATE OR REPLACE VIEW iceberg.brain_serving.mv_gold_customer_health AS
SELECT
  brand_id,
  brain_id,
  recency_days,
