...
// (brain_serving.semantic_{customer,order,product,campaign,journey}) that Wave D lays over the
...
//   D1-2  Row-level tenancy is enforced through the ${BRAND_PREDICATE} seam (withServingBrand injects
//         `brand_id = ?`) — a scoped read returns ONLY the requested brand's rows (the serving HTTP API has
//         no row policy, AMD-07 D3 — the predicate injection IS the compile-time tenancy) AND the seam refuses
...
//
// The suite cleanly PENDINGs when duckdb-serving is unavailable / the serving views aren't provisioned (never
// a hard failure on a missing engine) — mirrors contribution-margin.live.test.ts. Requires duckdb-serving on
// :8091 with the db/iceberg/duckdb/views/semantic_*.sql applied (the replica applies them at epoch build).
import { describe, it, expect, beforeAll } from 'vitest';
import { createDuckDbServingPool } from './duckdb-serving-adapter.js';
import { withServingBrand, BRAND_PREDICATE } from './serving-deps.js';
...
async function anyBrandWithRows(view: string): Promise<string | null> {
  const rows = await withServingBrand<Array<{ brand_id: string }>>(
    serving,
    '00000000-0000-0000-0000-000000000000',
    (scope) =>
      scope.runScoped<{ brand_id: string }>(
        `SELECT brand_id FROM brain_serving.${view} WHERE ${BRAND_PREDICATE} LIMIT 1`,
      ),
...
beforeAll(async () => {
  ...
  try {
    await serving.query('SELECT 1');
    // Confirm the serving views are provisioned (else PENDING rather than fail).
    await serving.query('SELECT count(*) FROM brain_serving.semantic_customer');
    servingUp = true;
  } catch {
    servingUp = false;
  }
...
describe('SPEC:D.1 — semantic entity views (thin compositions over mv_*/gold_*)', () => {
  it('D1-1/D1-2: every entity has brand_id first + BRAND_PREDICATE seam isolates one brand', async () => {
    if (!servingUp) {
      console.warn('[D.1] duckdb-serving unavailable — PENDING');
      return;
    }
    for (const view of SEMANTIC_VIEWS) {
      ...
      const scoped = await withServingBrand<Array<Record<string, unknown>>>(serving, brand, (scope) =>
        scope.runScoped(
          `SELECT * FROM brain_serving.${view} WHERE ${BRAND_PREDICATE} LIMIT 200`,
        ),
      );
  ...
  it('D1-2: seam is fail-closed — a query missing the ${BRAND_PREDICATE} sentinel is refused', async () => {
    if (!servingUp) return;
    await expect(
      withServingBrand(serving, '00000000-0000-0000-0000-000000000000', (scope) =>
        scope.runScoped('SELECT * FROM brain_serving.semantic_customer LIMIT 1'),
      ),
  ...
  it('D1-3: semantic_journey exposes deterministic-basis rows only (§1.4)', async () => {
    ...
    if (!brand) return;
    const bases = await withServingBrand<Array<{ identity_basis: string }>>(serving, brand, (scope) =>
      scope.runScoped<{ identity_basis: string }>(
        `SELECT DISTINCT identity_basis FROM brain_serving.semantic_journey WHERE ${BRAND_PREDICATE}`,
      ),
    );
  ...
  it('D1-4: semantic_order carries Wave-C economics (integer minor + currency) + journey pointer', async () => {
    ...
    if (!brand) return;
    const rows = await withServingBrand<Array<Record<string, unknown>>>(serving, brand, (scope) =>
      scope.runScoped(
  ...
  it('D1-5: semantic_customer.identity_basis is the honest deterministic constant', async () => {
    ...
    const bases = await withServingBrand<Array<{ identity_basis: string }>>(serving, brand, (scope) =>
      scope.runScoped<{ identity_basis: string }>(
        `SELECT DISTINCT identity_basis FROM brain_serving.semantic_customer WHERE ${BRAND_PREDICATE}`,
      ),
    );
