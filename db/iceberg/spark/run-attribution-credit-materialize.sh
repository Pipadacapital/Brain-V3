#!/usr/bin/env bash
# run-attribution-credit-materialize.sh — DB-AUDIT H2: materialize PG attribution_credit_ledger →
# Iceberg Bronze (brain_bronze.attribution_credit). Sibling of run-revenue-ledger-materialize.sh.
set -euo pipefail
...
echo "[attribution-credit-materialize] image=${SPARK_IMAGE} netns=container:${REDPANDA_CONTAINER}"
...
exec docker run --rm \
  ...
  /opt/spark/bin/spark-submit \
    ...
    /opt/spike/attribution_credit_materialize.py
