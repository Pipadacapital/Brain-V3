...
async function repullOrdersCursor(params: CursorRepullParams): Promise<number> {
  ...
  while (true) {
    ...
    // deterministic (uuidV5FromOrderLive over brand/order/modified_at), so a re-pull overlap
    // re-mints the SAME (brand_id, event_id) and is collapsed by Bronze compaction dedup + the
    // Silver MERGE.
