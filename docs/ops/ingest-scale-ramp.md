...
| **Transform brand-sharding** | split `duckdb-refresh` into N brand-hash shard workers (Argo, one worker per bucket; watermark+MERGE already shard-safe) | full Silver+Gold pass exceeds the 5-min window, OR any single job's window rows > ~5M | Embarrassingly parallel across brands (`brand_id`-first everywhere). A SINGLE hot brand outgrowing one shard is the only case needing intra-brand parallelism — escalate per ADR-0015 §5.7, do not pre-build. |
