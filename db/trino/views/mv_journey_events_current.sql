-- ============================================================
-- Brain V4 — Trino serving VIEW: mv_journey_events_current
--
-- Brain V4 serving runs over TRINO (Iceberg). This view is the thin serving projection over the
...
--
-- The metric-engine reads this as the two-part name brain_serving.mv_journey_events_current; with
-- the Trino default catalog = iceberg that resolves to iceberg.brain_serving.mv_journey_events_current.
-- brand_id is the tenant key; the ${BRAND_PREDICATE} seam injects brand_id = ? at read time.
-- ============================================================
CREATE OR REPLACE VIEW iceberg.brain_serving.mv_journey_events_current AS
SELECT
  brand_id,
  brain_id,
  touchpoint_id,
