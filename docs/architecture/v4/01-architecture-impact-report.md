...
| **Iceberg Bronze** | Raw truth, append-only, replayable, in Iceberg | `db/iceberg/spark/bronze_materialize.py` *(the writer at audit time — since replaced by the Kafka Connect sink, ADR-0010)*: Redpanda → idempotent **MERGE ... WHEN NOT MATCHED THEN INSERT** on `(brand_id,event_id)`; no UPDATE/DELETE. PG `bronze_events` dropped (0070); dormant `BronzeRepository` PG path default-OFF. | **CONFORMANT** (one fully-conformant medallion hop); ⚠️ remove dormant PG-Bronze path |
