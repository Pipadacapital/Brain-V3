...
| **SEV-2** | Serving/API down or wrong, data intact and still landing | Trino down (all dashboards 500); ArgoCD apps degraded; pgbouncer outage | Same day. Bronze keeps landing (Connect is independent of serving) — verify that FIRST, then fix serving. |
...
| **Redis** (ElastiCache) | analytics cache, stampede locks | snapshots retained 7d (modules/elasticache) — but treated as **disposable cache** | n/a (rebuildable) | cache warm-up (minutes–1h of elevated Trino load) | flush/replace node; nothing to restore | MEASURED |
...
5. **Verify** — Trino: `SELECT count(*) FROM iceberg.brain_bronze.<table>` + `SELECT * FROM iceberg.brain_bronze."<table>$snapshots" ORDER BY committed_at DESC LIMIT 5`; snapshot ids must match the catalog's current pointer; a few `mv_*` serving reads 200.
