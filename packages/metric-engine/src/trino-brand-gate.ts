// SPEC: D.3 / §1.11.3
/**
 * @brain/metric-engine — per-brand Trino admission gate (concurrency + FIFO queue + timeout).
 *
 * §1.11.3 requires a PER-BRAND Trino concurrency gate at the single serving chokepoint: one runaway
 * brand (a dashboard fan-out, a BAI storm) must not exhaust the shared Trino coordinator's slots and
 * starve every other tenant. The metric-engine has exactly ONE place every serving query flows
 * through: the TrinoPool. This module wraps ANY TrinoPool with a brand-keyed admission gate:
 *
 ...
 * the withTrinoBrand/withSilverBrand seam (it wraps the concrete pool), so the brand_id is read from
 * the LAST query param — the documented seam invariant: "the brand-isolation seam appends brandId as
 * the LAST param" (trino-deps.ts / trino-adapter.ts). A query with no string tail param (rare
 * operator ad-hoc) falls into a shared '__unkeyed__' bucket rather than bypassing the gate.
 *
 ...
 *
 * @see packages/metric-engine/src/trino-deps.ts    — the brand-scoped seam (appends brandId last)
 * @see packages/metric-engine/src/trino-adapter.ts — the concrete pool this wraps
 */
...
export interface PerBrandTrinoGateConfig {
  /**
   * Max simultaneously-running queries per brand_id. Default 8 — comfortably above a single
   * dashboard's parallel panel loads, low enough that one brand can't monopolize the coordinator.
   * Set from config/env at the composition root.
   */
...
export class TrinoGateRejectedError extends Error {
  ...
  constructor(brandKey: string, reason: 'queue_full' | 'acquire_timeout', detail: string) {
    super(`[trino-gate] query rejected for brand ${brandKey} (${reason}): ${detail}`);
    this.name = 'TrinoGateRejectedError';
...
/**
 * Wrap a TrinoPool with a per-brand admission gate. Returns a TrinoPool with the SAME interface,
 * so the composition root swaps it in transparently (createPerBrandTrinoGate(createTrinoPool(...))).
 *
 * @param pool   - the concrete pool to protect (e.g. from createTrinoPool).
 * @param config - gate limits (all optional; defaults are permissive).
 */
export function createPerBrandTrinoGate(
  pool: TrinoPool,
  config: PerBrandTrinoGateConfig = {},
): TrinoPool {
  ...
  function acquire(key: string): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      waiter.timer = setTimeout(() => {
        ...
        reject(
          new TrinoGateRejectedError(key, 'acquire_timeout', `waited ${acquireTimeoutMs}ms for a slot`),
        );
      }, acquireTimeoutMs);
      b.queue.push(waiter);
    });
