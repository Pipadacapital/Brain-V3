 ...
 *      with a stubbed fetch returning synthetic Insights rows → the WIRED SpendLedgerConsumer
 *      writes ad_spend_ledger rows. Proves: enumerate (SECURITY DEFINER) → GUC-after-enumerate
 *      → map (spend major→minor) → emit spend.live.v1 → consumer → ledger.
 *
 *   SM2 (Google throttle): a stubbed fetch returning RESOURCE_EXHAUSTED → the run marks the
 *      connector sync_status='error' (RateLimited) and ABORTS without writing spend (ADR-AD-7).
 *
 * The status surface reflects REAL connector_sync_status (never a simulated badge — ADR-AD-9).
 *
 * INFRA: Redpanda + Postgres. The SpendLedgerConsumer is started here (same class as main.ts).
 */
...
async function readSpendCount(appPool: Pool, platform: string): Promise<number> {
  const c = await appPool.connect();
  try {
    await c.query('BEGIN');
    await c.query(`SELECT set_config('app.current_brand_id', $1, true)`, [SMK_BRAND]);
    const r = await c.query<{ n: string }>(`SELECT count(*)::text AS n FROM ad_spend_ledger WHERE brand_id=$1 AND platform=$2`, [SMK_BRAND, platform]);
...
// ── SM1: Meta synthetic-fixture repull → ledger rows ──────────────────────────

describe('SM1: meta-spend-repull synthetic fixture → connect→repull→spend.live.v1→ad_spend_ledger', () => {
  it('runs the real meta repull with a stubbed Insights fetch → wired consumer writes spend rows', async () => {
    ...
    // Synthetic recorded-response fetch stub for the Graph API. NEVER hits the real network.
    const fetchStub = vi.fn(async (input: string | URL) => {
      ...
      if (url.includes('/insights')) {
        // Return one campaign-level row only on the first (campaign) call; empty otherwise.
    ...
    // The wired consumer should land the spend row.
    const count = await pollUntil(() => readSpendCount(appPool, 'meta'), (n) => n >= 1, 40_000);
    ...
    console.info('[SM1] PASS — meta synthetic repull → ad_spend_ledger row; sync_status=connected');
  }, 60_000);
});

// ── SM2: Google RESOURCE_EXHAUSTED → RateLimited + abort (no spend) ───────────

describe('SM2: google-ads-spend-repull RESOURCE_EXHAUSTED → RateLimited + abort run (ADR-AD-7)', () => {
  it('daily ops-quota error → sync_status=error (RateLimited), no spend rows written', async () => {
    if (!infraAvailable) return;
    await assertBrainApp(appPool);
    ...
    const beforeCount = await readSpendCount(appPool, 'google_ads');
    ...
    const fetchStub = vi.fn(async (input: string | URL) => {
      ...
      if (url.includes('searchStream')) {
        // Daily ops-quota exhausted (ADR-AD-7 DAILY branch) → abort run.
    ...
    const afterCount = await readSpendCount(appPool, 'google_ads');
    expect(afterCount).toBe(beforeCount); // no spend written under daily quota

    expect(await readSyncState(superPool, SMK_CI_GOOGLE)).toBe('error');
    console.info('[SM2] PASS — RESOURCE_EXHAUSTED → RateLimited abort, no spend; sync_status=error');
  }, 60_000);
});
