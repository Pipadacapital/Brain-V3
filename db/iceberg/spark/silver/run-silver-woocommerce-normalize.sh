#!/usr/bin/env bash
# run-silver-woocommerce-normalize.sh — ADR-0006 P4: normalize RAW WooCommerce orders in Spark Silver.
# Clone of run-silver-shopify-order-normalize.sh (job file + raw table changed). Reads
# brain_bronze.woocommerce_orders_raw (Kafka Connect Iceberg sink output) and produces the SHARED canonical
# order.live.v1 rows (source='woocommerce') via the GOLDEN-VECTOR-VERIFIED ports in _raw_normalize.py plus
# the Woo-local helpers in the job. Writes a SHADOW table by default (dual-run parity); set
# TARGET_TABLE=silver_collector_event at the order-lane cutover.
...
docker run --rm \
  ...
  /opt/spark/bin/spark-submit \
    ...
    "/opt/spark-src/silver/silver_woocommerce_normalize.py"

echo ""
echo "[silver] DONE — silver_woocommerce_normalize (raw WooCommerce orders → canonical Silver) ✓"
