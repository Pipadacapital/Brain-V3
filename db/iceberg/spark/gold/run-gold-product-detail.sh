#!/usr/bin/env bash
# run-gold-product-detail.sh — Brain V4 P3 (Spark Gold): build the NET-NEW gap Gold mart
# gold_product_detail (per-product funnel: views → add-to-cart → purchases → revenue + returns + rates;
# no dbt predecessor → parity status=NEW) FROM Iceberg Silver via Spark. ADDITIVE + idempotent +
# re-runnable; repoints NO reader.
#
#   gold_product_detail.py  (silver_order_line + silver_collector_event [product/cart pixel] +
#                            silver_return → per (brand_id, product_id) performance row)
#
...
# Requires the lakehouse profile (iceberg-rest + minio) up + Silver built
# (silver_order_line / silver_collector_event / silver_return).
#
# Usage:   db/iceberg/spark/gold/run-gold-product-detail.sh
...
echo "[gold-product-detail] image=${SPARK_IMAGE} netns=container:${REDPANDA_CONTAINER}"
echo "[gold-product-detail] packages=${PACKAGES}"

docker volume create brain-spark-ivy >/dev/null

echo "[gold-product-detail] >>> spark-submit gold_product_detail.py"
docker run --rm \
  ...
  /opt/spark/bin/spark-submit \
    ...
    "/opt/spark-src/gold/gold_product_detail.py"

echo "[gold-product-detail] DONE ✓"
