...
def build(spark: SparkSession):
    ...
    raw = rn.read_bronze(spark, CATALOG, BRONZE_NAMESPACE, "shiprocket_shipments_raw", "shiprocket")
    # Skip-guard: empty-lane skip — the shiprocket_shipments_raw_connect table auto-creates on the
    ...
    if raw.limit(1).count() == 0:
        print(f"[silver-shiprocket-normalize] {rn.connect_source_table(CATALOG, BRONZE_NAMESPACE, 'shiprocket_shipments_raw')} has 0 rows — skipping (empty lane; table auto-creates on first record, ADR-0010)", flush=True)
        return TARGET, 0
    ...
    out = canon.select(
        ...
        _event_category(lit("shiprocket.shipment_status.v1")).alias("event_category"),
