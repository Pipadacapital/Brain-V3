...
| **duckdb-serving** (`duckdb-serving`) | `kubectl -n duckdb-serving rollout restart deployment` | **Sole serving engine**, but stateless with ≥2 replicas — a ROLLING restart is a blip, not a gap (each new pod re-attaches the catalog + re-applies the views before `/readyz` goes green; gate on that). All replicas down at once = EVERY BFF analytics read 500s ("fetch failed" — looks like an app bug, isn't; see `investigate-oom.md`). Redis cache absorbs warm keys only. |
| **iceberg-rest** (`iceberg-rest`) | `kubectl -n iceberg-rest rollout restart deployment` | Catalog API. duckdb-serving epoch builds + DuckDB job starts fail during the gap (bounded retries absorb it); Connect keeps committing only if the catalog answers — keep the gap short. |
...
# landing alive (after kafka-connect/duckdb-serving/iceberg-rest restarts):
