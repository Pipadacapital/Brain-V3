-- ============================================================
-- Brain V4 — Trino serving VIEW: mv_silver_checkout_signal
--
...
--
-- The metric-engine reads this as the two-part name brain_serving.mv_silver_checkout_signal;
-- with the Trino default catalog = iceberg that resolves to
-- iceberg.brain_serving.mv_silver_checkout_signal. brand_id is the tenant key; the
-- ${BRAND_PREDICATE} seam injects brand_id = ? at read time.
-- ============================================================
CREATE OR REPLACE VIEW iceberg.brain_serving.mv_silver_checkout_signal AS
SELECT
  brand_id,
  event_id,
  signal_type,
