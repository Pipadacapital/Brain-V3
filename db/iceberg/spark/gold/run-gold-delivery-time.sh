#!/usr/bin/env bash
# run-gold-delivery-time.sh — Brain V4 Phase 3 (Spark Gold): build the NET-NEW gap Gold mart
# gold_delivery_time (delivery-time DISTRIBUTION per courier; no dbt predecessor → parity status=NEW)
# FROM Iceberg Silver via Spark. ADDITIVE + idempotent + re-runnable; repoints NO reader.
#
#   gold_delivery_time.py  (silver_shipment → per-(brand,courier) delivered dispatched→delivered
#                           whole-day gap → 0-1/2-3/4-5/6-7/8+ histogram + avg_delivery_days scalar)
#
...
# Requires the lakehouse profile (iceberg-rest + minio) up + Silver built (silver_shipment).
#
# Usage:   db/iceberg/spark/gold/run-gold-delivery-time.sh
...
echo "[gold-delivery-time] image=${SPARK_IMAGE} netns=container:${REDPANDA_CONTAINER}"
echo "[gold-delivery-time] packages=${PACKAGES}"

docker volume create brain-spark-ivy >/dev/null

echo "[gold-delivery-time] >>> spark-submit gold_delivery_time.py"
docker run --rm \
  ...
  /opt/spark/bin/spark-submit \
    ...
    "/opt/spark-src/gold/gold_delivery_time.py"

echo "[gold-delivery-time] DONE ✓"
