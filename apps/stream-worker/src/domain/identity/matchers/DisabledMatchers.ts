 ...
 *
 * Deterministic-first: probabilistic / ML / household / cross-device matching are ACKNOWLEDGED
 * in the registry but NOT implemented. Each is a `DisabledMatcher` (from @brain/contracts) with
...
/**
 * Probabilistic record linkage (Fellegi–Sunter weighted partial matches). DEFERRED.
 * id/version mirror IDENTITY_MATCHER_REGISTRY's 'probabilistic-fellegi-sunter' descriptor.
 */
export class ProbabilisticMatcher extends DisabledMatcher {
  constructor() {
    super('probabilistic-fellegi-sunter', 'v0', 'probabilistic');
  }
}
