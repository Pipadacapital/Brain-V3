-- ============================================================
-- Brain V4 — Trino serving VIEW: mv_gold_customer_scores
--
...
--
-- The metric-engine reads this as the two-part name brain_serving.mv_gold_customer_scores;
-- with the Trino default catalog = iceberg that resolves to
-- iceberg.brain_serving.mv_gold_customer_scores. brand_id is the tenant key; the
-- ${BRAND_PREDICATE} seam injects brand_id = ? at read time.
-- ============================================================
CREATE OR REPLACE VIEW iceberg.brain_serving.mv_gold_customer_scores AS
SELECT
  brand_id,
  brain_id,
  currency_code,
