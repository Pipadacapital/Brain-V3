...
def build(spark: SparkSession):
    ...
    # Keystone-mirrored idempotency (silver_collector_event.py Gap C): only overwrite when the payload
    # genuinely CHANGED, and bump silver_version on every real overwrite (coalesce so ALTER-ADDed
    # pre-existing shadow rows start from 1, never NULL+1=NULL).
    spark.sql(
        f"""
