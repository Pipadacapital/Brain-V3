...
| 6 | refresh | `ONESHOT=1 APP_ENV=local-prod pnpm dev:v4-refresh` → one-shot Silver→Gold (duckdb-serving picks the new marts up on re-query/rotation) | dashboards render honest-empty, not 500, on a cold DB |
...
| `pnpm dev:core` | core | infra (Postgres, Kafka, MinIO, Iceberg-REST, duckdb-serving, kafka-connect, …) + web/core deps |
...
> iceberg-rest, duckdb-serving, localstack, apicurio, pgbouncer), **full-obs** (tempo, loki,
...
`brain_bronze.collector_events_connect` (operational readers use the serving lift
...
`pnpm dev:v4-refresh` → `tools/dev/duckdb-refresh.sh` runs DuckDB Silver→Gold in
dependency order; the `brain_serving.mv_*` views need no refresh leg (duckdb-serving
sees new Iceberg commits on plain re-query). `ONESHOT=1` runs it once (used by step 6);
...
collector `http://localhost:8787`, duckdb-serving `http://localhost:8091`,
