#!/usr/bin/env bash
# run-silver-ad-spend-normalize.sh — ADR-0006 P4: normalize RAW ad-spend rows (Meta + Google) in Spark Silver.
# Reads brain_bronze.meta_spend_raw + brain_bronze.google_spend_raw (Kafka Connect Iceberg sink output) and
# produces the canonical spend.live.v1 rows via the GOLDEN-VECTOR-VERIFIED ports (shared _raw_normalize.py
# uuid_shaped + the connector-local money/level ports). Writes a SHADOW table by default (dual-run parity);
# set TARGET_TABLE=silver_collector_event at the spend-lane cutover.
# Ad spend has NO PII (I-S02) — there is NO per-brand salt / Postgres read here (unlike the Shopify exemplar).
set -euo pipefail
...
docker run --rm \
  ...
  -e TARGET_TABLE="${TARGET_TABLE:-silver_collector_event_ad_spend_shadow}" \
  -e META_ROW_FIELD="${META_ROW_FIELD:-insight}" \
  -e GOOGLE_ROW_FIELD="${GOOGLE_ROW_FIELD:-row}" \
...
echo ""
echo "[silver] DONE — silver_ad_spend_normalize (raw Meta + Google spend → canonical Silver) ✓"
