 ...
 *
 * The gate protects the single serving chokepoint (the TrinoPool) so ONE brand's fan-out cannot
...
import { createPerBrandTrinoGate, defaultBrandKeyOf, TrinoGateRejectedError } from './trino-brand-gate.js';
...
describe('D3 / §1.11.3 — per-brand Trino admission gate', () => {
  ...
  it('caps in-flight queries PER brand and lets a second brand run unaffected', async () => {
    ...
    const gate = createPerBrandTrinoGate(pool, { maxConcurrentPerBrand: 2 });
  ...
  it('serves queued waiters in FIFO arrival order', async () => {
    ...
    const gate = createPerBrandTrinoGate(pool, { maxConcurrentPerBrand: 1 });
    const order: number[] = [];
  ...
  it('rejects (queue_full) once concurrent + queue are saturated — fail-loud, bounded', async () => {
    const { pool, gates } = deferredPool();
    const gate = createPerBrandTrinoGate(pool, { maxConcurrentPerBrand: 1, maxQueuePerBrand: 1 });
    ...
    await expect(gate.query('q', [BRAND_A])).rejects.toBeInstanceOf(TrinoGateRejectedError);
  ...
  it('rejects (acquire_timeout) a query that cannot get a slot in time', async () => {
    ...
    try {
      const { pool, gates } = deferredPool();
      const gate = createPerBrandTrinoGate(pool, { maxConcurrentPerBrand: 1, acquireTimeoutMs: 1_000 });
  ...
  it('is default-permissive: under normal load it passes params through untouched', async () => {
    ...
    const gate = createPerBrandTrinoGate(pool); // shipped defaults
