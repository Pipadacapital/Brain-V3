#!/usr/bin/env bash
# run-gold-utm-source.sh — Brain V4 (Spark Gold, P3): build the NET-NEW Gold mart gold_utm_source
# (the UTM / acquisition-SOURCE matrix; no dbt predecessor → parity status=NEW) FROM Iceberg Silver
# (+ the optional sibling Gold gold_customer_360) via Spark. ADDITIVE + idempotent + re-runnable;
# repoints NO reader.
#
#   gold_utm_source.py  (silver_touchpoint first-touch utm/source/medium + silver_order_state revenue +
#                        gold_customer_360 lifetime_value/repeat → per-(brand,source,medium) matrix:
#                        visitors / conversions / revenue_minor / avg_ltv_minor / repeat_rate_pct)
#
...
# Requires the lakehouse profile (iceberg-rest + minio) up + Silver built (silver_touchpoint, silver_order_state).
#
# Usage:   db/iceberg/spark/gold/run-gold-utm-source.sh
...
echo "[gold-utm-source] image=${SPARK_IMAGE} netns=container:${REDPANDA_CONTAINER}"
echo "[gold-utm-source] packages=${PACKAGES}"

docker volume create brain-spark-ivy >/dev/null

echo "[gold-utm-source] >>> spark-submit gold_utm_source.py"
docker run --rm \
  ...
  /opt/spark/bin/spark-submit \
    ...
    "/opt/spark-src/gold/gold_utm_source.py"

echo "[gold-utm-source] DONE ✓"
