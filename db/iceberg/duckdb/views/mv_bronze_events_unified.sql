...
--
-- Lanes appear as their tables auto-create; the duckdb-serving epoch rotation re-applies views,
-- picking up a new lane after its first record; NOT a replay source of record — operational audit
-- surface only (exploded raw lanes cannot reproduce wire bytes). This is a LOCAL view in the
-- serving process's own brain_bronze schema (DuckDB cannot create views inside the REST catalog).
--
...
CREATE OR REPLACE VIEW brain_bronze.events_unified AS
SELECT
