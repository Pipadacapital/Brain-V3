-- SPEC:D.2 / §1.11.1 — Spark-maintained pre-agg for interactive metric refund_amount @ grain=week.
-- Additive MEASURES only; the non-additive metric value is derived at read in the view.
CREATE TABLE IF NOT EXISTS iceberg.brain_serving.preagg_refund_amount_week (
  brand_id varchar,
  period timestamp(6),
...
-- SPEC:D.2 / §1.11.1 — refresh (idempotent full rebuild) for iceberg.brain_serving.preagg_refund_amount_week. Run by the Spark refresh loop.
INSERT OVERWRITE iceberg.brain_serving.preagg_refund_amount_week
SELECT
    brand_id,
