...
WHY A SEPARATE HARNESS (not the Spark job)
  The production job runs inside the apache/spark:3.5.3 image (Python 3.8 → Splink 3.x,
  SparkLinker over Iceberg). This harness is a DEV/CI tool and runs on the developer's Python
  (3.9+ → Splink 4.x, DuckDB backend) directly over the golden CSVs — no Spark/Iceberg/live
  stack needed, so the model card is regenerable in seconds. The FEATURES and comparison
  structure mirror the job; the reported ceiling is a property of the DATA (feature entropy),
  which is invariant to the Splink major/backend.

  Dev-only deps (NOT added to any runtime image):  pip install 'splink>=4,<5' duckdb pandas

RUN
  python3 db/iceberg/spark/silver/splink_v1_golden_eval.py \
...
def main() -> int:
    ...
    recs = build_records(Path(args.baseline))
